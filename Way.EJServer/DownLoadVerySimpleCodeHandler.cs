...
{
    class DownLoadVerySimpleCodeHandler : Way.Lib.ScriptRemoting.ICustomHttpHandler
    {
        public void Handle(string originalUrl, HttpConnectInformation connectInfo, ref bool handled)
        {
            if (originalUrl.Contains("/DownLoadVerySimpleCodeHandler.aspx") == false)
                return;
            handled = true;
            try
            {
                ...
                using (EJDB db = new EJDB())
                {
                    ...
                    {
                        codeBuilder.BuildVerySimpleTable(db, codeNamespace, table);
                    }
